...
+function($){
  ...
  $.modal = function(params) {
    ...
    modalHTML = '<div class="modal ' + extraClass + ' ' + noButtons + '">'+
                '<div class="modal-inner">' + (titleHTML + textHTML + afterTextHTML) + 
                '</div><div class="modal-buttons ' + verticalButtons + '">' + 
                buttonsHTML + '</div></div>';
  ...
  $.openModal = function (modal, cb) {
    ...
    if (isToast) {
        modal.css({
            marginLeft: - Math.round(modal.width() / 2 / 1.185) + 'px' //1.185 是初始化时候的放大效果
        });
    }
